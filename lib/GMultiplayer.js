function GMP() {
                ...
                conn.on('data', (data) => {
                    if (!connectionAccepted) {
                        if (data) {
                            ...
                            conn.on('close', () => dataCallback(null));
                            conn.on('error', () => dataCallback(null));
                        } else {
                            connectionAccepted = true;
                            conn.close();
                            resolve(null);
    ...
    this.onConnection = async (callback, dataCallback) => {
        (await this.getMe()).on('connection', (conn) => {
            ...
            if (remoteGameId !== currentGameId) {
                return;
            }

            modals.yesNo(`${remoteName} want's to play you in online mode!`, 'Online request').then((response) => {
                if (response) {
                    conn.send(true);
