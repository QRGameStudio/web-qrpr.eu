...
function GSongLib() {
    ...
    this.downloadOnlineSongs = async () => {
        let response = await fetch(`https://api.${isLocalhost?'qrpr.eu':window.location.host}/music/library`);
        Object.assign(this.cache, await response.json());
        await storage.set('cache', this.cache);
    }
    ...
    this.get = async (name) => {
        ...
        if (!(name in this.cache)) {
            await this.downloadOnlineSongs();
        }
        if (!name in this.cache) {
            return null;
        }
    ...
    this.play = async (name, repeatCount=0) => {
        const seq = this.get(name);
        if (seq) {
            seq.play(repeatCount);
            return true;
        }
